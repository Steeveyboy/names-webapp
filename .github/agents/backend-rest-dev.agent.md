---
description: "Use this agent when the user asks to implement backend features, create REST services, handle data ingestion, or maintain backend code quality.\n\nTrigger phrases include:\n- 'implement a new backend feature'\n- 'create a REST endpoint for'\n- 'set up data ingestion'\n- 'build a backend API'\n- 'fix this backend bug'\n- 'improve backend code'\n- 'refactor this service'\n- 'add this feature to the backend'\n\nExamples:\n- User says 'The frontend team needs a new API endpoint to fetch user data' → invoke this agent to design and implement the REST endpoint\n- User asks 'Can you set up data ingestion for our CSV files?' → invoke this agent to create the data ingestion pipeline\n- During code review, user says 'This backend function is too complex and hard to follow' → invoke this agent to refactor for clarity\n- User requests 'Build a backend service to handle authentication' → invoke this agent to implement clean, maintainable authentication logic\n- User discovers 'There's a bug when processing large datasets' → invoke this agent to diagnose and fix the issue"
