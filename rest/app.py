...
def normalize_name(name: str) -> str:
    """Normalise user input to match SSA storage format (Title Case).

    SSA data stores every name with an initial capital and the rest
    lower-case (e.g. ``Mary``, ``James``).  Pre-processing the input
    here lets the DB layer use simple ``name = ?`` equality, which
    enables index usage and avoids per-row ``LOWER()`` calls.
    """
...
def get_name_records(name: str):
    ...
    return db.get_name_records(normalize_name(name))
...
def get_name_trends(name: str, gender: Optional[str] = Query(None, description="Filter by gender: M or F")):
    ...
    return db.get_name_trends(normalize_name(name), gender=gender)
...
def get_name_stats(name: str):
    ...
    stats = db.get_name_stats(normalize_name(name))
    if stats is None:
...
def get_gender_breakdown(name: str):
    ...
    return db.get_gender_breakdown(normalize_name(name))
...
def get_decade_trends(name: str, gender: Optional[str] = Query(None, description="Filter by gender: M or F")):
    ...
    return db.get_decade_trends(normalize_name(name), gender=gender)
...
def get_name_rank(
    ...
    rank = db.get_name_rank(normalized, year, gender)
    if rank is None:
        raise HTTPException(status_code=404, detail="Name not ranked in that year")
    return {"name": normalized, "year": year, "gender": gender, "rank": rank}
...
def get_name_by_state(name: str, state: Optional[str] = Query(None, description="Two-letter state code, e.g. CA")):
    ...
    return db.get_name_by_state(normalize_name(name), state=state)
...
def get_state_distribution(name: str):
    ...
    return db.get_state_distribution(normalize_name(name))
...
def search_names(
    ...
    return db.search_names(normalize_name(q), limit=limit)
